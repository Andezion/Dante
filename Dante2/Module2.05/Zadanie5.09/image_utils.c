...
struct image_t* image_flip_horizontal(const struct image_t *m1)
{
    ...
    *(m1->type + 0) != 'P' || *(m1->type + 1) != '2')
    ...
    for(int i = 0; i < image->height; i++)
    {
        *(image->ptr + i) = malloc(sizeof(int) * image->width);
        if(*(image->ptr + i) == NULL)
        {
            for(int j = 0; j < i; j++)
            {
    ...
    for(int i = m1->height - 1; i >= 0; i--)
    {
        for(int j = 0; j < m1->width; j++)
        {
            *(*(image->ptr + (m1->height - 1 - i)) + j) = *(*(m1->ptr + i) + j);
        }
    }
...
struct image_t* image_flip_vertical(const struct image_t *m1)
{
    ...
    for (int i = 0; i < m1->height; i++)
    {
        int temporary = m1->width - 1;
        for (int j = 0; j < m1->width; j++)
        {
            *(*(image->ptr + i) + j) = (*(*(m1->ptr + i) + temporary));
            temporary--;
        }
    }
    return image;
